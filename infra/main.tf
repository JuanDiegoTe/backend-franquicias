...
locals {
  user_data = <<-EOF
    #!/bin/bash
    # Log de todo lo que haga user-data
    ...
    echo "[2] instalar docker.io y git"
    apt-get install -y docker.io git

    echo "[3] habilitar y arrancar docker"
    systemctl enable docker
    systemctl start docker
    # Pequeño retraso para asegurar que el servicio esté listo
    sleep 5

    echo "[4] crear carpeta /opt/franquicias"
    mkdir -p /opt/franquicias
    cd /opt

    echo "[5] clonar repo (si no existe)"
    ...
    echo "[6] build + up de docker compose"
    # En Ubuntu 22.04/24.04 docker.io ya incluye docker compose v2
